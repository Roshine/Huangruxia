	...
	$.extend({		
		routes: {
			...
			default:function(route){
				...
				var real_hash = '';
				var real_param = '';
				var hash_arr = /(.*)\?(.*)/i.exec(route);
				
				if(!hash_arr || hash_arr.length != 3){
					real_hash = route;
				} else {
					real_hash = hash_arr[1];
					real_param = hash_arr[2];
				}

				if(real_hash[real_hash.length - 1] != '/')
					real_hash += '/';
			...
			load: function(hash) {
				...
				var real_hash = '';
				var real_param = '';
				var hash_arr = /#(.*)\?(.*)/i.exec(hash);
				
				if(!hash_arr || hash_arr.length != 3){
					real_hash = hash.substr(1);
				} else {
					real_hash = hash_arr[1];
					real_param = hash_arr[2];
				}
				...
				var route = $.routes.findRoute(real_hash);

				// route not found
				if (!route) { 
					if(real_hash != this.default_route){
						if(this.default_route.param.length)
							location.hash = this.default_route.hash + '?' + this.default_route.param;
						else
							location.hash = this.default_route.hash;
					}
					return; 
				}
