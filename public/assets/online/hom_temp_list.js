...
$(document).ready(function(){
	window.actionsEvents = {
			    'click .fillin': function (e, value, row, index) {//请求填写问卷页面，传送pretempid
			        window.location.href="#/hom_fillin?tempid="+row.homeworkTempId;
			    },
			    'click .subm_detail': function (e, value, row, index) {
			        window.location.href="#/hom_subm_detail?tempid="+row.homeworkTempId;
			    },
			    'click .only_view': function (e, value, row, index) {
			    	window.location.href="#/hom_only_view?tempid="+row.homeworkTempId;
			    }
	};	
	var templisttable=$('#templisttable');
    $.ajax({
        url:"homeworkTempListStu",
        type:'POST',
        dataType:'json',
