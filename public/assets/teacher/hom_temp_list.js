...
$(document).ready(function(){
	...
	$("#createTemp").click(function(){
		window.location.href="#/hom_create_temp";
	});
	$("#auto_create_temp").click(function(){
		window.location.href="#/hom_auto_create";
	});
	window.actionsEvents = {
			    'click .view': function (e, value, row, index) {
			        window.location.href="#/hom_stu_list?tempid="+row.id+"&published="+row.published;
			    },
			    'click .viewtemp': function (e, value, row, index) {
			    	if(row.published=='no'){
			    		if (row.type==1) {
			    			window.location.href="#/hom_auto_detail?tempid="+row.id+"&published=no";
			    		}else{
			    			window.location.href="#/hom_temp_detail?tempid="+row.id+"&published=no";
			    		}
			    	}else{
			    		if (row.type==0) {
			    			window.location.href="#/hom_temp_detail?tempid="+row.id+"&published="+row.published;
			    		}else{
			    			window.location.href="#/hom_auto_detail?tempid="+row.id+"&published="+row.published;
			    		}
			    	}
			    },
